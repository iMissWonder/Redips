<?php	
	...
	if(empty($jobidIn)){
			...
			echo "---------------------------------------------------------</br>";
			exec("curl http://".$scdHost.":".$scdPort."/cancel.json -d project=bodyExtract -d job=".$jobid,$out);
			echo "$out[0]";
						  }
	else{
			echo "你的jobid的是".$jobidIn."</br>现在我们一起来停止它！！！下面是返回值：</br>";
			echo "---------------------------------------------------------</br>";
			exec("curl http://".$scdHost.":".$scdPort."/cancel.json -d project=bodyExtract -d job=".$jobidIn,$out);
			echo "$out[0]";
	    }
?>