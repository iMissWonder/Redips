<?php
	...
	exec("curl http://".$scrapyid.":6800/cancel.json -d project=taobaoSearch -d job=".$jobid_redis,$out_redis);
	echo "$out_redis[0]</br>";
	echo "你的jobid的是".$jobid."</br>现在我们一起来停止它！！！下面是返回值：</br>";
	echo "---------------------------------------------------------</br>";
	//$ip = "http://192.168.199.203:6800/cancel.json";
	exec("curl http://".$scrapyid.":6800/cancel.json -d project=taobaoSearch -d job=".$jobid,$out);
	echo "$out[0]";
?>
