<?php	
	...
	if(empty($jobidIn)){
			...
			exec("curl http://".$scdHost.":".$scdPort."/cancel.json -d project=JDSearch -d job=".$jobid,$out);
			echo "$out[0]";
						  }
	else{
			...
			echo "---------------------------------------------------------</br>";
			//$ip = "http://scrapydtest.t2.daoapp.io:61031/cancel.json";
			exec("curl http://".$scdHost.":".$scdPort."/cancel.json -d project=JDSearch -d job=".$jobidIn,$out);
			echo "$out[0]";
	    }
?>